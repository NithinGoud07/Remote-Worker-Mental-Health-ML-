...
if menu == "📊 Dataset Overview":
    ...
    if st.button("Predict"):
        ...
        input_df = pd.DataFrame([[age, gender, region, industry, job_role, work_arrangement, hours_per_week,
                                  burnout_level, work_life_balance, "; ".join(physical_health_issues),
                                  social_isolation_score, salary_range]], columns=inp.columns)
        ...
        if hasattr(label_enc, 'inverse_transform'):
            ...
            class_names = label_enc.inverse_transform(range(len(prediction_proba[0])))
        else:
